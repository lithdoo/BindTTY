...
| § | 章节 | § | 章节 |
| --- | --- | --- | --- |
| [1](#_1-范围) | 范围 | [7](#_7-renderer-设计要点) | Renderer |
| [2](#_2-术语) | 术语 | [8](#_8-interaction-设计要点) | Interaction |
| [3](#_3-分层职责) | 分层职责 | [9](#_9-分阶段开发计划) | 分阶段计划 |
| [4](#_4-数据流) | 数据流 | [10](#_10-测试策略) | 测试策略 |
| [5](#_5-稳定接口契约) | 接口契约 | [11](#_11-示例应用-m7-完成后) | 示例应用 |
| [6](#_6-layout-设计要点) | Layout | [12](#_12-历史计划) | 历史计划 |
...
控件 API（VScrollView / HScrollView / ScrollView / List）见 [../widgets/SCROLL.md](../widgets/SCROLL.md)。
...
@bindtty/interaction
  scroll 容器或 VScrollView widget 消费 ↑/↓/PgUp/PgDn
  与 focus 链协同：focus 在内部可编辑控件上时优先交给 TextInput

@bindtty/widgets（推荐）
  VScrollView / List 作为高层 API，内部组合 box + metadata props + onKey
...
```tsx
import { VScrollView, List } from "@bindtty/widgets";
```
...
2. `VScrollView` 可以先编译成普通 `box`，通过内部 metadata props 驱动 layout / renderer。
3. 未来若需要 `<scroll>` intrinsic，可以在保持 `VScrollView` API 不变的前提下把内部实现替换为 intrinsic。
...
`VScrollView` / `HScrollView` / `ScrollView` / `List` 的 props、受控规则、`stickToBottom` / `showScrollbar`、键盘行为见 **[../widgets/SCROLL.md](../widgets/SCROLL.md)**。
...
5. `scrollX` / `scrollY` 由 layout clamp；widget 层通过 `HScrollView` / `VScrollView` 暴露受控 offset。
...
4. 如果 `for` 的 children 高度超过外层 `VScrollView`，由外层 box 的 clip 裁剪。
...
| 键 | 行为（VScrollView focused 或 scrollOnArrow） |
| --- | --- |
| ↑ | `offset -= 1`（clamp） |
| ↓ | `offset += 1` |
...
2. 若 focus 在 VScrollView 或可滚动容器上：方向键改 offset
3. 若 focus 在 scroll 内非输入子节点：M7 不做事件冒泡，只有当前 focused 节点收到 key
```

实现上：`VScrollView` 提供 `onKey`，在 `interaction` 中与 TextInput 相同模式；focus 进入 scroll 区域时由 Tab 顺序决定。

### 8.3 VScrollView onKey 规则

`VScrollView` 内部 box 只有在 `scrollOnArrow !== false` 且存在 `onOffsetChange` 时才挂载滚动 handler。
...
`@bindtty/terminal` 的 raw stdin 路径已支持 CSI / SS3 方向键与 Home / End / PageUp / PageDown 解析，也支持 Windows console prefixed navigation key。real PTY E2E 已覆盖 `VScrollView` / `List` 的 Down 键滚动；更细的组合键仍主要由 mock E2E 覆盖。
...
### 阶段 5：VScrollView widget 与键盘滚动

**目标**：用户可以通过 `VScrollView` 声明滚动窗口，并用 key 更新 offset。
...
- [x] 新增 `VScrollView`
- [x] `VScrollView` 渲染为内部 `box`
- [x] `height` / `width` / `overflow="clip"` / `scrollY` 转发到 box
- [x] `onKey` 支持 ↑/↓/PgUp/PgDn/Home/End
- [x] `onOffsetChange` 受控写回外部 signal
- [x] `@bindtty/widgets` 与 `bindtty` 入口导出 `VScrollView`

**验收**：

- [x] widgets 单测：`scrollOnArrow` / 无 `onOffsetChange` 时 focus 行为符合文档
- [x] mock E2E：focus 到 VScrollView 后 ↓ 改变可见行
- [x] mock E2E：TextInput focused 时方向键优先被 TextInput 消费，不滚动外层 VScrollView
...
| `@bindtty/widgets` | VScrollView props、onKey、List composition |
...
- [x] `VScrollView` 输出内部 box props
- [x] `VScrollView` onKey 调用 `onOffsetChange`
...
- [x] signal offset 更新
- [x] 键盘滚动
- [x] TextInput 与 VScrollView 同屏时方向键优先级
...
const app = createApp(
  <screen>
    <vstack>
      ...
      <VScrollView height={20} offset={scrollY}>
