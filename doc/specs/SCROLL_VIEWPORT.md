...
`stickToEnd` 规则镜像 §5.3.1：`left` detach；`end` 或 `right` 到达 `maxX` re-attach；`onLayout` 可在 `appliedX < maxX` 时调用 `onOffsetChange(maxX)`。
