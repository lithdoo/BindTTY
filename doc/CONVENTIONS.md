...
| 首页分工 | [index.md](./index.md) = 快速开始；[README.md](./README.md) = 全量索引；互链说明保持同步 |
| 长文导航 | 文内 **h2 ≥ 8** 的活跃文档（sidebar 内 architecture / packages / specs / widgets / testing）文首加 `::: info 本章导航`（勿用 `##`，避免重复出现在右侧 TOC）。§15+ 可用分组 bullet（如 DESIGN、VNODE、INTERACTION），其余优先双列表格；`LAYOUT_PROPS.md` 为生成文档，可省略。编号 h2 锚点须带 VitePress 前缀（如 `#_1-范围`，非 `#1-范围`）；`docs:build` 末尾运行 `scripts/check-doc-chapter-nav.mjs` 校验。侧栏 outline 仅展示 h2–h3（`outline.level: [2, 3]`） |
| OG / 分享 | Open Graph 等 meta 在 `doc/.vitepress/config.mts` 的 `head` 中维护 |
