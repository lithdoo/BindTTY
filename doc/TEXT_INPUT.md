...
MVP 不支持 `width`。当前 layout 仍把 `width` 视为 future layout prop，直接透传会触发 `Unsupported layout prop: width`。固定宽度、横向滚动、内容窗口等能力等 layout width 支持后再补。
...
const clampedCursor = computed(() => {
  return Math.min(cursor.get(), rawValue.get().length);
});
