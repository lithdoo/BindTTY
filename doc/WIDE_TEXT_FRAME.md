...
本文档是 [YOGA_LAYOUT.md](https://chatgpt.com/c/YOGA_LAYOUT.md) 的后续补充。`YOGA_LAYOUT.md` 中的 `@bindtty/text` 当前按 ASCII-first / plain text MVP 推进，暂不承诺 CJK、emoji、combining mark、grapheme cluster 的真实终端显示语义。本文档的目标是补齐这部分能力。

相关文档：

- [VNODE.md](https://chatgpt.com/c/VNODE.md) — Template / MountedNode 类型设计
- [RUNTIME.md](https://chatgpt.com/c/RUNTIME.md) — Template → MountedNode、binding、dirty、scheduler
- [LAYOUT.md](https://chatgpt.com/c/LAYOUT.md) — MountedNode → LayoutNode
- [RENDERER.md](https://chatgpt.com/c/RENDERER.md) — LayoutNode → Frame → ANSI Patch
- [YOGA_LAYOUT.md](https://chatgpt.com/c/YOGA_LAYOUT.md) — Text Measurement 与 Yoga Layout
- [NODE_SETUP.md](https://chatgpt.com/c/NODE_SETUP.md) — Element Ref / Layout Callback
- [M7_SCROLL_VIEWPORT.md](https://chatgpt.com/c/M7_SCROLL_VIEWPORT.md) — ScrollView / List viewport 与 scroll 数据流
...
```text
1. width = 0 时 char 强制为 ""。
2. width = 1/2 时 char 必须非空。
3. style shallow clone。
...
- 新增 `TextSegment`。
- 新增 `segmentText()`。
- `measureTextWidth()` 改为 display width。
- `measureText()` 使用 display width。
- 新增 `sliceTextByWidth()`。
- 保持 text value plain text。
- 不支持 ANSI escape。
...
- `hardWrapLine()` 按 segment width 切行。
- `wordWrapLine()` 按 display width 判断行宽。
- `truncateEnd()` 按 display width 截断。
- `truncateMiddle()` 按 display width 截断。
- `truncateStart()` 按 display width 截断。
- 所有操作不切断 grapheme。
...
- `Cell` 增加 `width: 0 | 1 | 2`。
- blank cell 使用 `char: " ", width: 1`。
- placeholder cell 使用 `char: "", width: 0`。
- wide leading cell 使用 `width: 2`。
- `createBlankCell()` 更新。
- `cloneCell()` 更新。
- 移除或改造 `normalizeChar(char[0])`。
- 新增 `isPlaceholderCell()`。
- 新增 `isWideLeadingCell()`。
- 更新 `frameToLines()`。
- 新增 `frameToDebugLines()`。
...
- `writeTextClipped()` 改为按 `segmentText()` 写。
- 写入宽字符时创建 leading + placeholder。
- 写入前清理目标区域已有 wide char。
- 如果写入落在旧 placeholder 上，清理旧 leading cell。
- 如果写入覆盖旧 leading cell，清理旧 placeholder。
- clipping 时不绘制半个 segment。
- `paintText()` 不再使用 `.slice(0, node.rect.width)`。
...
- `cellEquals()` 比较 `char`、`width`、`style`。
- diff 发现 wide-related cell 变化时扩大 dirty range。
- 旧 frame 中 wide char 被覆盖时，patch 覆盖完整范围。
- 新 frame 中 wide char 出现时，patch 输出 leading cell。
- ANSI encoder 跳过 `width: 0` placeholder。
- ANSI encoder 对 width 2 的 char 只输出一次。
- 清理旧 wide char 时输出足够 spaces。
...
- BasicLayoutEngine wrapped text 使用 display width。
- YogaLayoutEngine text measure 使用 display width。
- `contentSize.width` 使用 display columns。
- wrapped text in ScrollView 高度正确。
- resize rewrap 高度正确。
...
- 更新 `doc/RENDERER.md` 的 Frame/Cell 说明。
- 更新 `doc/YOGA_LAYOUT.md` 中 ASCII-first 的限制。
- 新增 `doc/WIDE_TEXT_FRAME.md`。
- 新增 CJK text 示例。
- 新增 emoji text 示例。
- 明确 ANSI escape 仍不属于 text value MVP。
