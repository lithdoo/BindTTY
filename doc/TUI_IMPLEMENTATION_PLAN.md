...
**M1–M6 主链路已完成**：TSX → ViewTemplate → MountedNode → LayoutNode → Frame (Cell) → ANSI Patch，含 terminal、interaction、Button、TextInput 与 `bindtty` createApp。

下一阶段重点为 **Milestone 7**（scroll / list / viewport）、高级 layout props、顶层 API 完善与 npm 发布准备。主链路示意：
...
- 全部包当前为 `private: true`、`0.1.0-alpha.0`。若未来发布 npm，需移除 private 并配置发布元数据。
...
**已完成（M1–M6）**：vnode → jsx-runtime → runtime → layout → renderer-terminal → terminal → interaction → widgets → bindtty createApp。

**下一步**：

1. scroll / list / viewport（Milestone 7）
2. 高级 layout props（width、height、gap、flex）
...
6. 宽字符 / CJK / emoji grapheme 支持
7. npm 发布准备

## 一句话方向

M1–M6 主链路已打通。下一阶段重点是 Milestone 7（scroll / list / viewport）、高级 layout、以及顶层 API 与示例完善；signal 更新继续以 binding-level invalidation 驱动 dirty、layout 和 paint。
