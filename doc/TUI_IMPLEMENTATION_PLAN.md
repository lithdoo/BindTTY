...
早期计划中 widgets 包定义 intrinsic element 的运行时行为，并承载 MVP 阶段的 focus / keyboard 能力。
...
button: 通过 onKey 转换 activate 语义，并负责绘制
input:  通过 onKey 转换编辑语义，并维护双向 value binding
~~~
...
stdin raw mode
  ...
  -> focused node onKey
  -> widgets / user component semantic handler
...
- button 语义：`@bindtty/widgets` 或用户组件
...
`value` 可以直接接收 `Signal<string>`。input 的编辑语义属于 widgets 或用户组件；keyboard dispatch 由 `@bindtty/interaction` 负责。
...
5. @bindtty/renderer-terminal：Frame / ANSI diff / paint
6. @bindtty/terminal：terminal lifecycle / viewport / keypress adapter
7. @bindtty/interaction：shared prop model / focus / onKey dispatch
8. @bindtty/widgets：button / input 等高层控件语义
9. bindtty：统一入口
10. scroll / list / viewport：TUI 核心组件（后续）
