...
当前正式模型为 **11 个可发布包** + 私有 `packages/e2e`。历史 7 包 / 10 包规划见 [archive/plans/PACKAGE_MODEL_HISTORY.md](../archive/plans/PACKAGE_MODEL_HISTORY.md)。
...
@bindtty/widgets:
  负责 Button / TextInput / ScrollView / List 等高层控件语义。
...
// + CreateAppOptions、widget props、Signal 相关类型
~~~

`createApp` 组合 runtime、layout、renderer、terminal、interaction，支持 stdout 与 terminal 双模式。JSX 通过 `./jsx-runtime` 子路径转发。

**不**从顶层导出 `runtime` / `vnode` / `layout` / `renderer-terminal`；高级扩展按需引用子包。`@bindtty/signal` 仍可独立安装。
...
1. CI 与 npm 发布执行
2. ~~顶层 `bindtty` API 冻结（signal re-export）~~（已完成；见 [packages/bindtty/README.md](../../packages/bindtty/README.md)）
