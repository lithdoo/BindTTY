...
| [总体目标](#总体目标) | 框架目标 |
| [Monorepo 包结构](#monorepo-包结构) | 12 包模型 |
| [核心分层](#核心分层) | 四层链路 |
...
**M1–M7 主链路已完成**：TSX → ViewTemplate → MountedNode → LayoutNode → Frame (Cell) → ANSI Patch，含 `@bindtty/input`、`@bindtty/terminal`、interaction、widgets（Button / Checkbox / TextInput / Textarea / Select / ScrollView 系列 / List / ProgressBar）与 `bindtty` createApp。
...
当前正式模型为 **12 个可发布包** + 私有 `packages/e2e`。历史 7 包 / 10 包 / 11 包规划见 [archive/plans/PACKAGE_MODEL_HISTORY.md（GitHub）](https://github.com/lithdoo/BindTTY/blob/main/doc/archive/plans/PACKAGE_MODEL_HISTORY.md)。
...
- 可发布包已移除 `private: true`，当前版本 `0.1.0-beta.0`（tags `beta`、`latest`）；根目录含 MIT `LICENSE`；`scripts/publish-packages.mjs` 按依赖顺序发布 12 包（含 `@bindtty/input`）。
...
6. ~~npm 发布准备~~（工程配置已完成；待 `npm run publish:packages`）
