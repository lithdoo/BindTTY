...
> **相关**：[APP.md](./APP.md) · [RENDERER.md](./RENDERER.md) · [INPUT.md](./INPUT.md)
...
| § | 章节 | § | 章节 |
| --- | --- | --- | --- |
| [1](#_1-背景) | 背景 | [9](#_9-输入模型) | 输入模型 |
| [2](#_2-目标) | 目标 | [10](#_10-与-createapp-的关系) | createApp |
| [3](#_3-包归属) | 包归属 | [11](#_11-与未来-widget-input-的关系) | Widget/Input |
| [4](#_4-包结构) | 包结构 | [12](#_12-错误处理) | 错误处理 |
| [5](#_5-对外-api) | 对外 API | [13](#_13-测试策略) | 测试策略 |
| [6](#_6-ansi-控制序列) | ANSI | [14](#_14-e2e-策略) | E2E |
| [7](#_7-生命周期) | 生命周期 | [15](#_15-落地阶段) | 落地阶段 |
| [8](#_8-viewport-与-resize) | Viewport | [16](#_16-当前结论) | 当前结论 |
...
2. cursor hide / show。
3. stdin raw mode。
4. input adapter 解析 raw stdin → interaction.handleKey。
...
7. 通过 `@bindtty/input` 解析 raw keyboard input。
...
9. 具体键盘协议解析规则；CSI / SS3 / Kitty / modifyOtherKeys 属于 `@bindtty/input`。
```
...
raw-input.ts
  兼容旧出口，内部转发到 @bindtty/input 的 parseInputChunk。

adapters/raw-stdin.ts
  持有 @bindtty/input createInputParser()，解析 raw stdin data chunk。

adapters/readline-stdin.ts
  基于 readline.emitKeypressEvents() 的兼容输入 adapter。
...
Terminal Host 支持两类 stdin adapter：

```text
ReadlineStdinInput
  readline.emitKeypressEvents(stdin)
  ↓
  normalizeKeypressEvent(input, key)
...
Readline adapter 只负责把 Node keypress object 转成 `TerminalKeyEvent`：

```ts
normalizeKeypressEvent(input, key);
```

### 9.2 Raw stdin parser

Raw adapter 不在 terminal 包内手写 CSI / Kitty / modifyOtherKeys 解析，而是复用 `@bindtty/input`：
