...
| § | 章节 | § | 章节 |
| --- | --- | --- | --- |
| [1](#_1-目标) | 目标 | [8](#_8-bindingvalue-规则) | BindingValue |
| [2](#_2-包位置) | 包位置 | [9](#_9-jsx-类型约束) | JSX 类型 |
| [3](#_3-typescript-配置) | TS 配置 | [10](#_10-实现伪代码) | 伪代码 |
| [4](#_4-必要导出) | 必要导出 | [11](#_11-包导出建议) | 包导出 |
| [5](#_5-输入形态) | 输入形态 | [12](#_12-测试计划) | 测试计划 |
| [6](#_6-转换规则) | 转换规则 | [13](#_13-mvp-验收标准) | 验收标准 |
| [7](#_7-children-规则) | children | | |
...
**注意：** 以下类型定义为文档简化版。实际代码中所有 intrinsic element 都会通过共享的 `IntrinsicInteractionProps` 获得 `id`、`focusable`、`onKeyCapture`、`onKey`、`onFocusChange`，且 `box`/`text` 等元素通过 `IntrinsicPaintProps` 额外获得 `color`、`background`、`bold`、`focusStyle` 等 paint prop。详情见各包源码。
