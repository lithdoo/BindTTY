...
本文后半部分关于 `ElementDefinition` 与 keyboard input system 的内容属于前瞻架构设计，不代表当前已落地代码。

::: info 本章导航

- **概述**：[§1 主链路](#_1-主链路) · [§2 包职责](#_2-包职责) · [§3 设计目标](#_3-设计目标) · [§4 核心原则](#_4-核心原则)
- **Template 节点**：[§5 节点集合](#_5-template-节点集合) · [§6 Empty](#_6-emptytemplate) · [§7 Element](#_7-elementtemplate) · [§8 Container/Leaf](#_8-为什么-elementtemplate-不区分-container-leaf) · [§9 IntrinsicTag](#_9-intrinsicelementtag) · [§10 Props](#_10-templateprops)
- **绑定与 children**：[§11 BindingValue](#_11-bindingvalue) · [§12 文本](#_12-文本设计) · [§13 children](#_13-children-规则)
- **控制流 Template**：[§14 Fragment](#_14-fragmenttemplate) · [§15 Component](#_15-componenttemplate) · [§16 Show](#_16-showtemplate) · [§17 For](#_17-fortemplate)
- **Schema 与 Normalize**：[§18 Element Schema](#_18-element-schema) · [§19 JSX 类型](#_19-jsx-类型约束) · [§20 TSX Normalize](#_20-tsx-normalize-规则) · [§21 示例](#_21-template-示例)
- **MountedNode 基础**：[§22 定位](#_22-mountednode-的定位) · [§23 为什么需要](#_23-为什么需要-mountednode) · [§24 与 Template 关系](#_24-mountednode-和-template-的关系) · [§25 核心职责](#_25-mountednode-的核心职责) · [§26 节点类型](#_26-mountednode-的节点类型)
- **ElementDefinition 与职责**：[§27 ElementDefinition](#_27-elementdefinition-基础控件能力的来源) · [§28 text 渲染](#_28-text-的渲染在哪里处理) · [§29 可编辑控件](#_29-可编辑控件的交互在哪里处理) · [§30 show/for](#_30-show-for-的职责) · [§31 Layout/Paint/Input](#_31-layout-paint-input-与-mountednode-的关系)
- **更新与对比**：[§32 Dirty](#_32-dirty-更新模型) · [§33 dispose](#_33-dispose-机制) · [§34 整体理解](#_34-推荐的整体理解) · [§35 vs React/DOM](#_35-和-react-dom-的区别) · [§36 边界](#_36-template-与-mountednode-边界) · [§37 类型草案](#_37-完整类型草案) · [§38 推荐写法](#_38-推荐用户写法) · [§39 总结](#_39-总结)
...
TextInput 等可编辑控件由 `@bindtty/widgets` 实现，挂载后仍是普通 `MountedElementNode` 树（通常是 `box` / `text` 组合）。
...
keyboard event
  ↓
InteractionController 接收事件
  ↓
找到当前 focused node
  ↓
调用该 node 的 onKey（capture → target → bubble）
  ↓
widgets / 业务组件更新本地 state
  ↓
触发绑定回调或用户事件
  ↓
标记 dirty
...
可编辑控件的交互逻辑不在 Template 层，也不应该散落在 scheduler 中，而应在 widget 或业务组件的 `onKey` 内实现。
...
InputSystem 则通过 InteractionController 找到 focused MountedElementNode，再调用该节点的 onKey。具体 Button / TextInput 语义由 widgets 或业务组件在 onKey 内实现。
