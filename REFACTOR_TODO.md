...
- [ ] 为 `layoutText` 缓存增加容量上限。
- [ ] 为 `measureText` 缓存增加容量上限。
- [ ] 选择并记录 LRU 或 App/Renderer 实例缓存策略。
- [ ] 同时定义最大条目数与字符数/估算字节预算，避免少量超大字符串占满内存。
- [ ] 增加高基数字符串输入下缓存不会无限增长的测试。
- [ ] 增加少量超大字符串输入下缓存预算仍然生效的测试。
- [ ] 增加缓存命中与淘汰后的结果一致性测试。
- [ ] 提供仅供测试/诊断的缓存 size、clear 或 stats 能力，避免测试依赖模块私有状态。
