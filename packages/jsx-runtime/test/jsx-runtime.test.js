...
import { jsxDEV } from '../dist/jsx-dev-runtime.js';
...
test('creates intrinsic element templates', () => {
  ...
  assert.equal(view.props.value, title);
});
...
test('creates component templates without executing component', () => {
  ...
  assert.equal(view.component, Header);
...
test('creates for templates with render functions', () => {
  ...
  const renderItem = (item) => jsx('text', { value: item.title });
  const view = jsx('for', {
    each: items,
    key: (item) => item.id,
    children: renderItem
  ...
  assert.equal(view.kind, 'for');
  assert.equal(view.each, items);
  assert.equal(view.key(items.get()[0], 0), 1);
  assert.deepEqual(view.renderItem(items.get()[0], 0), {
    kind: 'element',
    tag: 'text',
    props: {
      value: 'One'
