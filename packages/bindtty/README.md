...
当前公开版本为 `0.1.0-beta.2`；`bindtty`、`@bindtty/widgets`、`@bindtty/terminal` 等官方包建议保持同版本安装。
...
推荐统一从 `bindtty` 导入 signal，勿单独安装另一版本的 `@bindtty/signal`。使用 widgets 时，`bindtty` 与 `@bindtty/widgets` 请保持同版本号发布（如均为 `0.1.0-beta.2`）。
...
修复：
  ...
  4. 应用 package.json overrides: { "@bindtty/signal": "0.1.0-beta.2" }
```
