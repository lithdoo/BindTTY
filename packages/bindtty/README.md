...
推荐统一从 `bindtty` 导入 signal，勿单独安装另一版本的 `@bindtty/signal`。使用 widgets 时，`bindtty` 与 `@bindtty/widgets` 请保持**同版本号**发布（如均为 `0.1.0-alpha.4`）。
...
修复：
  ...
  4. 应用 package.json overrides: { "@bindtty/signal": "0.1.0-alpha.4" }
```
