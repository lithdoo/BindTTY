...
MVP 阶段收敛为 **12 个包**。text measurement 独立为 `@bindtty/text`，layout 默认使用 Yoga backend，renderer 已独立为 `@bindtty/renderer-terminal`，terminal lifecycle 独立为 `@bindtty/terminal`，raw keyboard input parsing 独立为 `@bindtty/input`，键盘 focus 与 `onKey` 派发独立为 `@bindtty/interaction`，调度器先放在 `@bindtty/runtime`。
...
| `@bindtty/input` | 终端 raw keyboard input tokenizer / parser，输出标准化 key event |
...
| `bindtty` | 框架入口：`createApp`、signal re-export、JSX 转发（不含 widgets） |
...
packages/input/
  src/tokenizer.ts    # raw chunk → text/control/CSI/SS3/paste token
  src/parse-token.ts  # token → InputEvent（Kitty / modifyOtherKeys / CSI）
  src/keymap.ts
  src/events.ts

packages/widgets/
  src/form/           # Button、Checkbox、TextInput、Select
  src/textarea.ts     # Textarea 组件导出
  src/textarea/       # Textarea 多行编辑 / Ctrl+Enter submit
...
@bindtty/input:             raw keyboard input tokenizer/parser（CSI/SS3/Kitty/paste）
@bindtty/terminal:          TerminalHost（alt screen/cursor/raw mode/resize/input adapter）
@bindtty/interaction:       keyboard focus、onKey dispatch、Tab/Shift+Tab traversal
@bindtty/widgets:           Button、Checkbox、TextInput、Textarea、Select、VScrollView、HScrollView、ScrollView、List、ProgressBar
